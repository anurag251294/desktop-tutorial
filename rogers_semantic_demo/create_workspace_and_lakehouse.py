"""Create Rogers_Semantic_Layer_Demo workspace on the corp Fabric trial
capacity, then a lakehouse for the Finance / ARPU demo. Persists IDs to
stack_finance.json.

Mirrors rogers_demo/create_workspace_and_lakehouse.py.
"""
...
# Same corp Fabric trial capacity used by CTC, Interac, and Rogers Network demos.
CAPACITY_ID = "fc2a4dac-2edb-4f33-9209-b0dac6a67c5d"
WS_NAME = "Rogers_Semantic_Layer_Demo"
LH_NAME = "rogers_finance_lh"
OUT = Path(__file__).parent / "stack_finance.json"
    ...
    if ws_id:
        ...
        s, h, b = call("POST", f"{API}/v1/workspaces",
                       body={"displayName": WS_NAME,
                             ...
                             "description": "Rogers Enterprise Semantic Layer demo - Finance ARPU one-measure-everywhere"})
        if s in (200, 201):
    ...
    if lh_id:
        ...
        s, h, b = call("POST", f"{API}/v1/workspaces/{ws_id}/lakehouses",
                       body={"displayName": LH_NAME,
                             "description": "Rogers Finance revenue / subscribers / churn / cost facts for ARPU semantic model"})
        if s == 202:

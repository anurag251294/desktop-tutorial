"""Create CanadianTire_Demo workspace on the existing corp Fabric trial capacity,
then a lakehouse inside it. Writes stack_ctc.json with all IDs.
"""
...
# Reuse the capacity we already activated (from Interac stack_corp.json)
CAPACITY_ID = "fc2a4dac-2edb-4f33-9209-b0dac6a67c5d"
WS_NAME = "CanadianTire_Demo"
LH_NAME = "ctc_lh"
OUT = Path(__file__).parent / "stack_ctc.json"
    ...
    if ws_id:
        ...
        s, h, b = call("POST", f"{API}/v1/workspaces",
                       body={"displayName": WS_NAME,
                             ...
                             "description": "Canadian Tire Merch Copilot demo - 2026-05-29"})
        if s in (200, 201):
    ...
    if lh_id:
        ...
        s, h, b = call("POST", f"{API}/v1/workspaces/{ws_id}/lakehouses",
                       body={"displayName": LH_NAME,
                             "description": "Merch SKU + Inventory + In-Season data"})
        if s == 202:

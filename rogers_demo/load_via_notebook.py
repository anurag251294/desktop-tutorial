"""Load Rogers CSVs to Delta via a Fabric Spark notebook.

The `tables/{name}/load` LRO sits in NotStarted on busy capacity. Spark
notebooks reliably get a session and write Delta in ~3-5 minutes.

Two-step Fabric pattern (per Fabric REST gotchas memory):
  1. POST /workspaces/{ws}/notebooks  with just displayName -> empty notebook
  2. POST /notebooks/{id}/updateDefinition  -> attach .ipynb + .platform

Then runOnDemand the notebook, poll until completion, refreshMetadata so
SM picks up the new Delta tables.
"""
...
def call(method, url, body=None, accept_redirect=True):
...
def poll_lro(loc, label="op", attempts=180, delay=5):
    """Poll an LRO Location header URL."""
...
def build_ipynb():
    """Build a single-cell ipynb that loads every table."""
    code_lines = [
...
PLATFORM = {
    ...
    "metadata": {"type": "Notebook", "displayName": NB_NAME,
                 "description": "Loads Rogers CSVs from Files/csv into Delta tables"},
...
def create_notebook():
    ...
    s, h, b = call("POST", f"{API}/v1/workspaces/{WS}/notebooks",
                   body={"displayName": NB_NAME,
                         "description": "Loads Rogers CSVs to Delta"})
    if s == 202:
        loc = h.get("location")
...
def run_notebook(nb_id):
    ...
    if s == 202:
        loc = h.get("location")
        # Find the job-instance id from the body or location
        # Poll the location until status terminal
        for i in range(180):
            time.sleep(10)
            try:
...
def main():
    print(f"== Loading Rogers data via Spark notebook ==")
